...
//             readOnly 
//             className="w-full bg-white border ${errors.post_title ? 'border-red-500' : 'border-gray-300" 
//           />
//           <input 
//             type="hidden" 
//             {...register("player_id")} 
...
function ReturnEquipment() {
  ...
  const inputStyle = (field) =>
    `w-full bg-white border ${errors[field] ? 'border-red-500' : 'border-gray-300'} 
    placeholder:text-slate-400 text-slate-700 text-sm rounded-md px-3 py-2.5 shadow-sm transition duration-300 
    focus:outline-none focus:border-2 focus:border-black hover:border-2 hover:border-black focus:shadow`;

  return (
    <Dashboard>
      <ToastContainer />
      <div className="min-h-screen p-4">
        <div className="w-full max-w-md mx-auto"> {/* Narrower form */}
          {loading ? (
            <div className="flex flex-col items-center justify-center h-screen">
              <RotatingLines
                visible={true}
            ...
            equipment.map((item, index) => (
              <form
                key={index}
                onSubmit={handleSubmit(handleReturnEquipment)}
                className="bg-white p-6 rounded-lg shadow-md space-y-4 w-full"
              >
                <div className="text-center mb-6">
                  <h3 className="text-xl sm:text-2xl font-extrabold tracking-tight text-gray-900">
                    Return <span className="text-indigo-600">Equipment</span>
                  </h3>
                </div>
                ...
                <div className="grid grid-cols-1 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      defaultValue={item.player_name}
                      readOnly
                      className={inputStyle('player_name')}
                    />
                    <input type="hidden" {...register("player_id")} defaultValue={item.player_id} />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      defaultValue={item.coach_name}
                      readOnly
                      className={inputStyle('coach_name')}
                    />
                    <input type="hidden" {...register("coach_id")} defaultValue={item.coach_id} />
                  </div>
                  ...
                  <div>
                    ...
                    />
                    <input type="hidden" {...register("equipment_request_id")} defaultValue={id} />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      defaultValue={item.equipment_name}
                      readOnly
                      className={inputStyle('equipment_name')}
                    />
                    <input
                      type="hidden"
                      defaultValue={item.equipment_name_id}
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      {...register("return_note")}
                      className={inputStyle('return_note')}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      defaultValue={item.return_date_time}
                      {...register("return_date_time")}
                      className={inputStyle('return_date_time')}
                    />
                  </div>
                </div>
                ...
                <div className="flex justify-end space-x-4 mt-4">
                  <button
                    type="button"
                    onClick={() => navigate(-1)}
                    className="rounded-md px-4 py-2 hover:border-indigo-900 border"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="rounded-md bg-indigo-600 py-2 px-4 border border-transparent text-sm text-white shadow-md hover:shadow-lg focus:bg-indigo-700 active:bg-indigo-700 disabled:pointer-events-none disabled:opacity-50"
                  >
                    Return
                  </button>
                </div>
