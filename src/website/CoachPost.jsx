...
//               Coach <span className="text-indigo-600"></span>
//             </h3>
//           </div>
...
//           className="bg-gray-100 border border-gray-800 p-2 rounded-lg lg:w-60 sm:w-auto sm:flex-grow focus:outline-none focus:ring-2 focus:ring-black"
...
function CoachPost() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const uniqueLevels = [...new Set(fetchedPosts.map(item => item.coach.level))];
    ...
    <>
      {loader ? (
        ...
        <div>
          ...
          <div className="text-center mt-10">
            <h3 className="text-3xl sm:text-4xl leading-normal font-extrabold tracking-tight text-gray-900">
              All <span className="text-indigo-600">Coaches</span>
            </h3>
          </div>
          ...
          <div className="flex justify-end mr-10 mt-4">
            <button
              ...
              className="flex items-center justify-center bg-indigo-600 text-white font-medium p-2 rounded-lg hover:bg-indigo-500 transition-all duration-1000"
            >
          ...
          {/* Filter Section */}
          <div className={`transition-all duration-1000 ease-in-out transform ${toggle ? "max-h-screen opacity-100" : "max-h-0 opacity-0"}`} style={{ overflow: "hidden" }}>
            <div className="flex flex-wrap justify-center gap-4 px-4 lg:px-36 sm:px-10 mt-4 relative">
              <input
                type="text"
                value={searchTerm}
                placeholder="Search by name"
              ...
              <button
                onClick={handleReset}
                title="Clear Filters"
                className="text-black text-3xl hover:text-white hover:bg-black transition-colors duration-300 px-3 rounded-lg"
              >
                ×
              </button>
            </div>
          ...
          <div className="grid grid-cols-1 gap-6 p-4">
            {filteredPosts.map((item) => (
              <div key={item.id} className="relative flex flex-col md:flex-row w-full bg-white shadow-2x1 border border-slate-200 rounded-lg">
                <div className="relative w-full sm:w-6/12 md:w-4/12 lg:w-3/12 shrink-0 overflow-hidden mx-auto p-3">
                  <img
                    src={`http://127.0.0.1:8000/uploads/coach_image/${item.coach.image}`}
                    alt="Coach"
                ...
                <div className="p-6">
                  <h4 className="text-slate-800 text-xl font-semibold">{item.coach.name}</h4>
                  <p className="text-slate-600 leading-relaxed text-sm bg-gray-100 px-4 py-2 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300">
                    {item.post_description}
                  </p>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 p-4">
                    <div className="flex items-center justify-center">
                      <p className={`inline-block text-sm font-semibold px-4 py-2 rounded-full shadow-md hover:shadow-lg transition-all duration-300 ${
                        item.coach.level === "level 1"
                          ? "bg-green-700 text-white border border-green-500"
                          : item.coach.level === "level 2"
                          ? "bg-yellow-400 text-gray-900 border border-yellow-500"
                    ...
                    <div>
                      {item.coach?.ratingreviews?.length ? (() => {
                        const totalRatings = item.coach.ratingreviews.reduce((sum, review) => sum + review.rating, 0);
                        const averageRating = totalRatings / item.coach.ratingreviews.length || 0;
                        ...
                        return (
                          <div className="flex flex-col items-center">
                            ...
                            <div className="text-sm text-gray-600">{item.coach.ratingreviews.length} reviews</div>
