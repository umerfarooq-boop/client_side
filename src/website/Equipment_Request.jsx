...
function Equipment_Request() {
  ...
  // Handle equipment select change
  const handleSelectChange = (selected) => {
    setSelectedEquipment(selected || []);
    const updatedQuantities = {};
    (selected || []).forEach((item) => {
      updatedQuantities[item.value] = quantities[item.value] || '';
    });
    setQuantities(updatedQuantities);
  };
  ...
  return (
    <>
      <Nav />
      <ToastContainer />
      <div className="bg-white border shadow sm:rounded-lg max-w-md mx-auto p-8 my-20">
        <h1 className="text-2xl font-bold text-blue-900 text-center">Equipment Request</h1>
        <form onSubmit={handleSubmit(ReqEquip)} className="mt-6 flex flex-col gap-4">
          
          <input type="hidden" {...register("player_id")} value={player_id} />
          <input type="hidden" {...register("coach_id")} value={coach_record} />
          <input type="hidden" {...register("equipment_status")} value="reject" />
          ...
          {/* Return DateTime */}
          <div>
            <label className="block font-medium">Return Date & Time</label>
            <input
              type="datetime-local"
